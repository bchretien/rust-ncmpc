...
impl<'c, 'm> Controller<'c, 'm>
{
    pub fn new(model: &'c mut SharedModel<'m>, config: &'c Config) -> Controller<'c,'m>
    {
        ...
        // Clear the playlist
        callbacks.insert(config.keys.clear.keycode(),
                         Box::new(playlist_clear));
        // Pause
        callbacks.insert(config.keys.play_pause.keycode(),
                         Box::new(playlist_pause));
        // Stop
        callbacks.insert(config.keys.stop.keycode(),
                         Box::new(playlist_stop));
        // Previous song
        callbacks.insert(config.keys.previous_song.keycode(),
                         Box::new(playlist_previous));
        // Next song
        callbacks.insert(config.keys.next_song.keycode(),
                         Box::new(playlist_next));
